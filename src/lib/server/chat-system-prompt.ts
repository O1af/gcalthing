...
function buildInstructions(executionMode: ExecutionMode, signedIn: boolean): string {
  const lines = [
    ...
    '- Use search_events for events outside the schedule window, or when you need to find something specific. It returns full details (attendees with RSVP status, description, organizer, links) when 5 or fewer events match. When more results come back, narrow the query or date range to get full details.',
